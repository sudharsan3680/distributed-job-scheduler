...
\begin{tikzpicture}[
    font=\small,
    ent/.style={rectangle, draw=rule, minimum width=1.9cm, minimum height=0.8cm, align=center, fill=panel},
    arr/.style={-, thick, black!65}
  ]
  \node[ent] (org)   at (0,0)       {Organization};
  \node[ent] (proj)  at (4.0,0)     {Project};
  \node[ent] (queue) at (8.0,0)     {Queue};
  \node[ent] (job)   at (12.0,0)    {Job};

  \node[ent] (worker) at (3.8,-3.0) {Worker};
  \node[ent] (batch)  at (5.9,-3.0) {Batch};
  \node[ent] (retry)  at (8.0,-3.0) {RetryPolicy};
  \node[ent] (sched)  at (10.1,-3.0){ScheduledJob};
  \node[ent] (exec)   at (12.2,-3.0){JobExecution};

  \node[ent] (log)   at (10.0,-6.0) {JobLog};
  \node[ent] (dlq)   at (14.4,-6.0) {DeadLetter};
